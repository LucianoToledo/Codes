...
SELECT f.nombre FROM fabricante f, producto p WHERE f.codigo = p.codigo_fabricante;

/*6/10. Lista el código de los fabricantes que tienen productos en la tabla producto, sin
mostrar los repetidos.*/

SELECT codigo_fabricante FROM producto GROUP BY codigo_fabricante;

/*7/11. Lista los nombres de los fabricantes ordenados de forma ascendente.*/

SELECT nombre FROM fabricante ORDER BY nombre;

/*8/12. Lista los nombres de los productos ordenados en primer lugar por el nombre de
...
/*9/13. Devuelve una lista con las 5 primeras filas de la tabla fabricante.*/

SELECT * FROM fabricante LIMIT 5;

/*10/14. Lista el nombre y el precio del producto más barato. (Utilice solamente las cláusulas ORDER BY y LIMIT)*/

SELECT nombre, precio FROM producto ORDER BY precio LIMIT 1;

/*11/15. Lista el nombre y el precio del producto más caro. (Utilice solamente las cláusulas ORDER BY y LIMIT)*/

SELECT nombre, precio FROM producto ORDER BY precio DESC LIMIT 1;

/*12/16. Lista el nombre de los productos que tienen un precio menor o igual a $120.*/

SELECT nombre, precio FROM producto WHERE precio <= 120;

/*13/17. Lista todos los productos que tengan un precio entre $60 y $200. Utilizando el operador BETWEEN.*/

SELECT * FROM producto WHERE precio BETWEEN 60 AND 200;

/*14/18. Lista todos los productos donde el código de fabricante sea 1, 3 o 5. Utilizando el operador IN.*/

SELECT * FROM producto WHERE codigo_fabricante IN (1,3,5);

/*14/23. Devuelve una lista con el nombre de todos los productos que contienen la cadena Portátil en el nombre.*/
...
SELECT p.codigo AS cod_prd, p.nombre, f.codigo AS cod_fab, f.nombre FROM fabricante f, producto p WHERE(p.codigo_fabricante = f.codigo);
...
SELECT p.codigo AS cod_prd, p.nombre, f.codigo AS cod_fab, f.nombre FROM fabricante f, producto p WHERE(p.codigo_fabricante = f.codigo) ORDER BY f.nombre;
...
SELECT p.* , f.* from producto p, fabricante f WHERE (p.codigo_fabricante = f.codigo) AND f.nombre LIKE 'Lenovo';

/*5. Devuelve una lista de todos los productos del fabricante Crucial que tengan un
precio mayor que $200.*/

SELECT p.* from producto p, fabricante f WHERE (p.codigo_fabricante = f.codigo) AND f.nombre LIKE 'Crucial' AND p.precio >200;

/*6. Devuelve un listado con todos los productos de los fabricantes Asus, Hewlett-
Packard. Utilizando el operador IN.*/

SELECT p.* , f.* from producto p, fabricante f WHERE (p.codigo_fabricante = f.codigo) AND f.nombre IN('Asus', 'Hewlett-Packard');
...
SELECT p.nombre, p.precio, f.nombre FROM producto p, fabricante f WHERE (p.codigo_fabricante = f.codigo) AND p.precio >= 180 ORDER BY p.precio DESC, f.nombre;
...
SELECT nombre FROM producto WHERE codigo_fabricante = (SELECT codigo FROM fabricante WHERE nombre = 'Asus') and precio > (SELECT AVG(precio) FROM producto);
...
SELECT f.nombre FROM producto p, fabricante f WHERE (p.codigo_fabricante = f.codigo) GROUP BY f.nombre 
HAVING COUNT(p.codigo_fabricante) = (SELECT COUNT(*) FROM producto WHERE codigo_fabricante = 2);
